...
server.admin("""box.space._cluster:update(1, {{
    {{'=', 3, (require('datetime').parse('{time}'))}},
    {{'=', 4, '{uuid}'}},
    {{'=', 5, (require('datetime').parse('{time}'))}},
    {{'=', 6, '{uuid}'}}}})""".format(time=new_time, uuid=new_uuid))
...
server.admin("""box.space._cluster:insert{{5, '{0}',
    require('datetime'):new(), require('uuid').NULL:str(),
    require('datetime'):new(), require('uuid').NULL:str()}}""".format(new_uuid))
