...
g.test_no_direct_connection = function(g)
    local term = g.follower1:exec(function() return box.info.election.term end)
    ...
    t.helpers.retrying({}, function()
        g.follower1:exec(function(t)
            require('luatest').assert(box.info.election.leader_idle >= t)
        end, {DEATH_TIMEOUT})
    end)
    t.assert_equals(
        g.follower1:exec(function() return box.info.election.term end), term,
        'No elections when leader is seen through some proxy node')
    g.follower1:exec(function(cfg) box.cfg(cfg) end,
